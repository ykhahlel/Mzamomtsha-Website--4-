...
Math.minmax = (value, limit) => {
  ...
  //Array to store Medium level black holes co-ordinates
  const mediumHoles = [
    { column: 0, row: 5 },
    { column: 2, row: 4 },
    { column: 6, row: 8 },
  ...
  //Array to store Hard level black holes co-ordinates
  const hardHoles = [
    { column: 0, row: 5 },
    { column: 2, row: 0 },
    { column: 2, row: 4 },
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (![" ", "H", "h", "M", "m", "E", "e"].includes(event.key)) 
    return;
  ...
  function resetGame() {
    ...
    if (hardMode) {
      noteElement.innerHTML = `Click the joystick to start!
          <p>Hard mode, Avoid black holes.fgdgf</p>`;
    }
    if (mediumMode) {
      noteElement.innerHTML = `Click the joystick to start!
          <p>Medium mode, Avoid black holes.</p>`;
    } else {
      noteElement.innerHTML = `Click the joystick to start!
          <p>Easy Mode, Move every ball to the center.</p>`;
    }
    ...
    // Reset hole elements if medium mode
    if (mediumMode) {
      mediumHoles.forEach(({ x, y }) => {
        const ball = document.createElement("div");
    ...
    try {
      ...
      if (
        ...
        noteElement.innerHTML = `Congrats, you did it!
          ${!hardMode ? "<p>Time for a new difficulty?</p>" : ""}
          <p>
